...
import {
  CheckSquare, ChevronDown, FolderOpen, LayoutDashboard, Plus, Tag, X,
...
export default function Sidebar({ onClose, mobile }: SidebarProps) {
  ...
  const stats = useMemo(() => {
    ...
    const breached  = tasks.filter(t => getSLAStatus(t) === 'breached').length
    const critical  = tasks.filter(t => getSLAStatus(t) === 'critical').length
    const atRisk    = tasks.filter(t => getSLAStatus(t) === 'at_risk').length
    return { total, done, inProg, breached, critical, atRisk }
  }, [state.tasks])
    ...
    <aside className={cn(
      ...
      <div className="flex items-center justify-between px-5 h-14 border-b border-slate-200 shrink-0">
        <div className="flex items-center gap-2.5">
          ...
          <span className="text-base font-bold text-slate-900 tracking-tight">TaskPro</span>
        </div>
        {mobile && (
      ...
      <div className="flex-1 overflow-y-auto py-3 px-2 space-y-0.5">
        ...
        <div className="pt-3">
          <button
            className="w-full flex items-center gap-2 px-3 py-1 text-xs font-semibold text-slate-400 uppercase tracking-wider hover:text-slate-600 transition-colors"
                ...
                <div className="px-2 pt-2 pb-1 space-y-2">
                  ...
                  <div className="flex gap-1 flex-wrap">
                    {PROJECT_COLORS.map(c => (
                      <button
                        key={c}
                        className={cn('w-5 h-5 rounded-full transition-transform hover:scale-110', newColor === c && 'ring-2 ring-offset-1 ring-slate-400')}
                  ...
                  <div className="flex gap-2">
                    <Button size="sm" variant="primary" onClick={handleAddProject}>Add</Button>
                    <Button size="sm" variant="ghost" onClick={() => setAddingProject(false)}>Cancel</Button>
                  </div>
                </div>
              ) : (
                <button
...
function ProjectItem({ project, active, count, onClick }: {
  project: Project; active: boolean; count: number; onClick: () => void
}) {
  return (
    <button
      onClick={onClick}
