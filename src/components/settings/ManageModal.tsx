...
import { Pencil, Trash2, Plus, X, Check, FolderOpen, Tag, CircleDot, GripVertical, Layers } from 'lucide-react'
import {
...
import type { Project, Label, StatusDef, TaskTemplate, Priority } from '../../types'
...
interface ManageModalProps {
  open: boolean
  onClose: () => void
  initialTab?: 'projects' | 'labels' | 'statuses' | 'templates'
}
...
export default function ManageModal({ open, onClose, initialTab = 'projects' }: ManageModalProps) {
  ...
  const t = useT()
  const [tab, setTab]               = useState<'projects' | 'labels' | 'statuses' | 'templates'>(initialTab)
    ...
    <Modal open={open} onClose={onClose} size="md">
      ...
      <div className="flex border-b border-slate-200 dark:border-slate-700 px-4 overflow-x-auto">
        {[
          ...
          { key: 'templates', icon: Layers,     label: state.language === 'vi' ? 'Mẫu' : 'Templates', count: (state.templates ?? []).length },
...
function TemplatesTab() {
  ...
  const empty: Omit<TaskTemplate, 'id'> = { name: '', description: '', priority: 'medium', estimatedHours: null, slaHours: null, labels: [], subtasks: [] }
  const [form, setForm] = useState<Omit<TaskTemplate, 'id'>>(empty)
  const [subtaskInput, setSubtaskInput] = useState('')

  const openEdit = (tpl: TaskTemplate) => { setEditing(tpl); setForm({ name: tpl.name, description: tpl.description, priority: tpl.priority, estimatedHours: tpl.estimatedHours, slaHours: tpl.slaHours, labels: tpl.labels, subtasks: tpl.subtasks, isBuiltin: tpl.isBuiltin }); setSubtaskInput(''); setAddingNew(false) }
