...
async function handleChat(body: Record<string, unknown>): Promise<Response> {
  ...
  const geminiRes = await fetch(GEMINI_URL, {
    method: 'POST',
    headers: { 'Authorization': `Bearer ${key}`, 'Content-Type': 'application/json' },
    body: JSON.stringify({ model: MODEL, messages: geminiMessages, max_tokens: 2048, stream: true }),
  })

  if (!geminiRes.ok || !geminiRes.body) {
    const err = await geminiRes.text()
    return new Response(JSON.stringify({ error: `Gemini error ${geminiRes.status}: ${err.slice(0, 200)}` }), {
      status: 502, headers: { ...CORS_HEADERS, 'Content-Type': 'application/json' },
    })
  }

  // Translate Gemini's OpenAI-compatible SSE to our data: {"text":"..."} format
